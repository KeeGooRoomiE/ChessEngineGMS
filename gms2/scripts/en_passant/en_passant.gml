...
if argument0=0
	{
		...
		if (Returned!=noone)
		{
			...
			if (Returned.Piece_ID=0) and (Returned.Color="White") and (ID >= 32) and (ID<= 39) and (Moves=1) and (Returned.Enpass = 1)
				{
				Returned.Enpass = 1
...
if argument0=1
	...
	{
		global.Passant_BLeft = Returned
		if (Returned.Piece_ID=0) and (Returned.Color="Black") and (ID >= 24) and (ID<= 31) and (Moves=1) and (Returned.Enpass = 1)
			{
			Returned.Enpass=1
			Returned=collision_point(x-sprite_width*0.5,y-sprite_height*0.5,obj_cell,false,true)
			Returned.Passant=1
